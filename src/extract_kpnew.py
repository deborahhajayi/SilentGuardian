...
# =============================
RAW_DIR = "data/raw"
CUSTOM_RAW_DIR = "data/custom_no_fall"
...
# =============================
# Extraction (CUSTOM NO-FALL)
# =============================
def extract_custom(video_path, video_id, writer):
    cap = cv2.VideoCapture(video_path)
    if not cap.isOpened():
        print("⚠ Cannot open", video_path)
        return 0
    ...
    while True:
        ...
        writer.writerow(
            [video_id, frame_idx, 0] +   # ALWAYS label 0
            kp.reshape(-1).tolist()
        )
        written += 1
...
# =============================
def main():
    out_csv = os.path.join(OUT_DIR, "all_keypoints_with_custom.csv")
    ...
    with open(out_csv, "w", newline="") as f:
        ...
        # =============================
        # ORIGINAL DATASET (REQUIRES ANNOTATIONS)
        # =============================
        ...
        # =============================
        # CUSTOM NO-FALL DATASET
        # =============================
        custom_video_dir = os.path.join(CUSTOM_RAW_DIR, "Videos")

        if os.path.isdir(custom_video_dir):
            custom_videos = [v for v in os.listdir(custom_video_dir)
                             if v.lower().endswith((".mp4", ".avi"))]

            for vid in custom_videos:
                video_id = "custom_" + os.path.splitext(vid)[0]

                rows = extract_custom(
                    os.path.join(custom_video_dir, vid),
                    video_id,
                    writer
                )
